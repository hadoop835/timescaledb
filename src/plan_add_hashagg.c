...
/* Definitions of functions with a custom group estimate.
   Will be sorted be function OID after initialization. */
...
static int
cmp_hashagg_func_by_oid(const void *left, const void *right)
{
	const CustomEstimateForFunctionInfo *funcl = (CustomEstimateForFunctionInfo *) left;
	const CustomEstimateForFunctionInfo *funcr = (CustomEstimateForFunctionInfo *) right;

	if (funcl->function_oid < funcr->function_oid)
		return -1;
...
initialize_custom_estimate_func_info()
{
	...
	for (i = 0; i < _MAX_INTERNAL_FUNCTIONS; i++)
	{
		...
		FuncCandidateList funclist = FuncnameGetCandidates(
														   list_make1(makeString(def.function_name)),
														   def.nargs, NIL, false, false, false);

		/* check types */
		while (!found && funclist != NULL)
		{
	...
	/* sort by OID */
	qsort(custom_estimate_func_info, _MAX_HASHAGG_FUNCTIONS, sizeof(CustomEstimateForFunctionInfo), cmp_hashagg_func_by_oid);
}

static CustomEstimateForFunctionInfo *
get_custom_estimate_func_info(Oid function_oid)
{
	CustomEstimateForFunctionInfo def =
	{
		.function_oid = function_oid
	};

	if (custom_estimate_func_info[0].function_oid == 0)
		initialize_custom_estimate_func_info();

	return bsearch(&def, custom_estimate_func_info, _MAX_HASHAGG_FUNCTIONS, sizeof(CustomEstimateForFunctionInfo), cmp_hashagg_func_by_oid);
}
...
{
	Node	   *first = eval_const_expressions(root, linitial(expr->args));
	Expr	   *second = lsecond(expr->args);
	Const	   *c;
	Interval   *interval;
	double		max_period,
				interval_period;
	...
	interval_period = (double) get_interval_period_approx(interval);

	max_period = estimate_max_spread_expr(root, second);
...
custom_group_estimate_date_trunc(PlannerInfo *root, FuncExpr *expr, double path_rows)
{
	...
	interval_period = (double) date_trunc_interval_period_approx(interval);
...
custom_group_estimate_integer_division(PlannerInfo *root, Oid opno, Node *left, Node *right)
{
	...
	if (strlen(function_name) == 1 && function_name[0] == '/' && IsA(right, Const))
	{
		...
		period = estimate_max_spread_expr(root, (Expr *) left);
		if (IS_VALID_ESTIMATE(period))
			return period / c->constvalue;
	}
	return INVALID_ESTIMATE;
}
...
custom_group_estimate_expr(PlannerInfo *root, Node *expr, double path_rows)
{
	if (IsA(expr, FuncExpr))
	{
		...
		CustomEstimateForFunctionInfo *hafd = get_custom_estimate_func_info(custom_group_estimate_func->funcid);

		if (NULL != hafd)
			return hafd->custom_group_estimate_func(root, custom_group_estimate_func, path_rows);
	}
					  ...
					  double path_rows)
{
	...
	foreach(lc, groupExprs)
	{
		...
		if (IS_VALID_ESTIMATE(estimate))
		{
			...
			newGroupExpr = lappend(newGroupExpr, item);
	}
	...
	if (newGroupExpr != NIL)
		dNumGroups *= estimate_num_groups(root, newGroupExpr, path_rows, NULL);

	if (dNumGroups > path_rows)
		return INVALID_ESTIMATE;
	return clamp_row_est(dNumGroups);
}
...
plan_add_parallel_hashagg(PlannerInfo *root,
						  ...
						  RelOptInfo *output_rel, double dNumGroups)
{
	Query	   *parse = root->parse;
	...
	double		dNumPartialGroups = custom_group_estimate(root,
														  cheapest_partial_path->rows);

	/* don't have any special estimate */
	if (!IS_VALID_ESTIMATE(dNumPartialGroups))
		return;
	...
	add_partial_path(output_rel, partial_path);
				 ...
				 RelOptInfo *output_rel)
{
	...
	dNumGroups = custom_group_estimate(root,
									   cheapest_path->rows);

	/* don't have any special estimate */
	if (!IS_VALID_ESTIMATE(dNumGroups))
		return;
	...
	if (try_parallel_aggregation)
		plan_add_parallel_hashagg(root, input_rel, output_rel, dNumGroups);
